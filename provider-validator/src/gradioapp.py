    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not providers:
                ...
                breakdown = debug.get("status_breakdown", {})
                bd_lines  = ["  - `" + s + "`: **" + str(c) + " records**" for s,c in breakdown.items()]
                bd_str    = "  \n".join(bd_lines) if bd_lines else "  - (empty — no records in DB yet)"
                return (
                    f"ℹ️ **No providers found** with filter: `{status_filter}`\n\n"
                    f"**Records in DB by status:**\n{bd_str}\n\n"
                    f"**DB total:** {debug.get('db_total','?')}  |  "
                    f"**CSV total:** {debug.get('csv_total','?')}  |  "
                    f"**CSV exists:** {debug.get('csv_exists','?')}\n\n"
                    "**What to do:**\n"
                    "- Orchestrator writes `confirmed` → use **validated** filter\n"
                    "- Orchestrator writes `manual_review` → use **flagged** filter\n"
                    "- To see everything: use **all** filter\n"
                    "- If DB total = 0: run batch first, then Refresh"
                )
...
| Dropdown Value | Shows |
|---|---|
| `all` | Every record in DB + CSV (use this after CLI batch) |
| `validated` | Only `status=confirmed` — high confidence, no flags |
| `flagged` | Only `status=manual_review` — has flags or low confidence |
| `pending` | Records with `confidence=0` — not yet enriched |
| `processing` | Records currently being processed |
""")
            with gr.Row():
                provider_limit   = gr.Slider(10, 100, value=20, step=10, label="Records to Display")
                status_filter_dd = gr.Dropdown(
                    choices=["all", "validated", "flagged", "pending", "processing"],
                    value="all",
                    label="Status Filter",
                    info="Orchestrator writes: confirmed=validated, manual_review=flagged"
                )
